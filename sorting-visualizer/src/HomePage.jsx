    ...
    const tick = () => {
      setBars(prev => {
        ...
        if (stepRef.current > 1) {
          next[(stepRef.current - 2 + COUNT) % COUNT].color = T.barDefault;
        }
        next[i].color = T.compare;
...
const ALGOS = [
  { key: "bubble",    name: "Bubble Sort",    time: "O(n²)",      best: "O(n)",       space: "O(1)",     color: T.compare },
  { key: "selection", name: "Selection Sort", time: "O(n²)",      best: "O(n²)",      space: "O(1)",     color: T.swap    },
  { key: "insertion", name: "Insertion Sort", time: "O(n²)",      best: "O(n)",       space: "O(1)",     color: "#3b82f6" },
  { key: "merge",     name: "Merge Sort",     time: "O(n log n)", best: "O(n log n)", space: "O(n)",     color: T.sorted  },
  { key: "quick",     name: "Quick Sort",     time: "O(n log n)", best: "O(n log n)", space: "O(log n)", color: T.accent  },
  { key: "heap",      name: "Heap Sort",      time: "O(n log n)", best: "O(n log n)", space: "O(1)",     color: T.pivot   },
];
...
const FEATURES = [
  ...
  { icon: "🎛️", label: "FULL CONTROLS",        desc: "Array size 5–100, speed slider, pause mid-sort, reset anytime." },
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div  style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ width: 30, height: 30, background: "linear-gradient(135deg, #6366f1, #8b5cf6)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 17, letterSpacing: "-0.02em", cursor: "pointer" }}>
              SortViz
            </span>
          </Link>
          <span style={{ color: T.textMuted, fontSize: 12, marginLeft: 4 }}>— algorithm visualizer</span>
          
        </div>
        </Link>
        <button className="hp-cta" onClick={onStart} style={{ padding: "7px 14px", fontSize: 12 }}>
          ▶ Launch App
        </button>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px" }}>
        ...
        <section className="hero-grid" style={{
          ...
          <div>
            <div className="fu1" style={{
              ...
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: T.sorted, animation: "blink 1.4s infinite" }} />
              <span style={{ fontSize: 10, color: T.accent, fontWeight: 700, letterSpacing: "0.1em" }}>
            ...
            {/* Stat cards — s */}
            <div className="fu5" style={{ display: "flex", gap: 10, marginTop: 32, flexWrap: "wrap" }}>
              {[
                { label: "ALGORITHMS",  val: "6",    color: T.accent  },
                { label: "MAX ARRAY",   val: "100",  color: T.compare },
                { label: "OPEN SOURCE", val: "YES",  color: T.sorted  },
              ].map(({ label, val, color }) => (
                <div key={label} style={{
                  background: T.surface,
                  ...
                  <span style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.08em", fontWeight: 600 }}>{label}</span>
          ...
          <div className="fu3" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <span style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.08em", fontWeight: 600 }}>LIVE PREVIEW</span>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: T.sorted, animation: "statusPulse 1s infinite" }} />
                <span style={{ fontSize: 10, color: T.sorted, fontWeight: 600 }}>RUNNING...</span>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 12 }}>
              {[
                { label: "COMPARISONS", val: (tick * 23 + 108).toLocaleString(), color: T.compare },
                { label: "SWAPS",       val: (tick * 9  + 44).toLocaleString(),  color: T.swap    },
                { label: "SORTED",      val: `${Math.min(tick * 5 + 8, 100)}%`,  color: T.sorted  },
              ].map(({ label, val, color }) => (
                <div key={label} style={{
            ...
            {/* Legend — exact copy from visualizer */}
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              {[["Comparing", T.compare], ["Swapping", T.swap], ["Sorted", T.sorted], ["Pivot", T.pivot]].map(([label, color]) => (
        ...
        <section ref={algosSectionRef} style={{ paddingBottom: 56, scrollMarginTop: 72 }} >
          <span style={{ fontSize: 10, color: T.textMuted, letterSpacing: "0.08em", fontWeight: 700, display: "block", marginBottom: 16 }}>
            ALGORITHMS
          </span>
          <div className="algo-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
            {ALGOS.map((a, i) => (
              <div key={a.name}
                ...
                style={{ borderColor: hoveredAlgo === i ? a.color : T.border }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14 }}>{a.name}</span>
                  <span className="tag" style={{
                    background: hoveredAlgo === i ? `${a.color}22` : T.surfaceAlt,
                    color: hoveredAlgo === i ? a.color : T.textMuted,
                    transition: "all 0.18s",
                  }}>{a.time}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {[
                    { k: "BEST CASE",  v: a.best,  c: T.sorted  },
                    { k: "WORST CASE", v: a.time,  c: T.swap    },
                    { k: "SPACE",      v: a.space, c: T.accent  },
                  ].map(({ k, v, c }) => (
                    <div key={k} style={{
                      ...
                      <span style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.06em", fontWeight: 600 }}>{k}</span>
          ...
          <div className="feat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
            {FEATURES.map(f => (
              <div key={f.label} className="hp-feat-card">
                ...
                <div style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.08em", fontWeight: 700, marginBottom: 6 }}>{f.label}</div>
                <div style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.7 }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </section>
      ...
      <footer style={{ borderTop: `1px solid ${T.border}`, background: T.surface }}>
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "space-between",
          flexWrap: "wrap", gap: 20, height: 64,
        }}>
          {/* Logo — */}
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
            ...
            <span style={{ color: T.textMuted, fontSize: 12 }}>— algorithm visualizer</span>
          </div>

          {/* Built by */}
          <div>
            <span style={{ fontSize: 10, color: T.textMuted, letterSpacing: "0.06em" }}>BUILT BY </span>
            <span style={{
              fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14,
              background: "linear-gradient(90deg, #6366f1, #8b5cf6)",
              WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
            }}>Abhay </span> 
             <span style={{ fontSize: 10, color: T.textMuted, letterSpacing: "0.06em" }}>(UID: 1253394105)</span>
          </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 10, color: T.textMuted, letterSpacing: "0.06em" }}>
